...
/**
 * A custom {@link AuthenticationProvider} to validate JWT auth tokens. The token authentication
 * provider has the following responsibilities,
 *
 * <p>1. Verify and validate the access token signature. 2. Create {@link OneOpsUser} by extracting
 * identity and auth claims from token. 3. Throws Authentication exception if the token is invalid,
 * malformed or expired.
 *
 * <p>{@link AuthenticationFailureHandler} is invoked upon failed authentication.
 *
 * @author Suresh G
 */
@Component

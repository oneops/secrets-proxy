 ...
 * <code>appName</code> is the OneOps environment name with <b>{org}_{assembly}_{env}</b> format,
 * for which you are managing the secrets.
 *
 * @author Suresh
...
public class GroupController {
  ...
  public GroupController(
      KeywhizAutomationClient kwClient, SecretService secretService, AuditLog auditLog) {
  ...
  public GroupDetailResponseV2 info(
      @PathVariable(APP_NAME_PARAM) String appName, @CurrentUser OneOpsUser user)
      throws IOException {
  ...
  public List<ClientDetailResponseV2> getClients(
      @PathVariable(APP_NAME_PARAM) String appName, @CurrentUser OneOpsUser user)
      throws IOException {
    AppGroup appGroup = new AppGroup(user.getDomain(), appName);
  ...
  public ClientDetailResponseV2 getClientDetails(
      @PathVariable(APP_NAME_PARAM) String appName,
      @PathVariable("clientName") String name,
      @CurrentUser OneOpsUser user)
      throws IOException {
  ...
  public void deleteClient(
    ...
    if (user.hasRole(ADMIN)) {
      ...
      log.error(
          String.format(
              "User: %s don't have %s permission to delete client: %s for %s",
              user.getUsername(), ADMIN, name, appGroup.getGroupName()));
      throw new KeywhizException(
          FORBIDDEN.value(),
          format("User %s don't have permission to delete the client.", user.getUsername()));
    }
  }
  ...
  public List<SecretDetailResponseV2> getSecrets(
      @PathVariable(APP_NAME_PARAM) String appName, @CurrentUser OneOpsUser user)
      throws IOException {
    AppGroup appGroup = new AppGroup(user.getDomain(), appName);
  ...
  public List<String> deleteAllSecrets(
      @PathVariable(APP_NAME_PARAM) String appName, @CurrentUser OneOpsUser user)
      throws IOException {
    AppGroup appGroup = new AppGroup(user.getDomain(), appName);
    List<AppSecret> appSecrets =
        kwClient
            .getSecrets(appGroup.getKeywhizGroup())
            .stream()
            .map(s -> new AppSecret(s.name()))
  ...
  public List<String> getSecretsExpiring(
      @PathVariable(APP_NAME_PARAM) String appName,
   ...
   * Creates new secret.
   *
   * @param createGroup <code>true</code> to create non existing application group. Default is
   *     <code>false</code>.
   ...
   * @throws IOException Throws if the request could not be executed due to cancellation, a
   *     connectivity problem or timeout.
   * @throws KeywhizException Throws if application group doesn't exist or the secret with the same
   *     name already exists.
   */
  ...
  public void createSecret(
      @RequestParam(value = "createGroup", required = false, defaultValue = "false")
          boolean createGroup,
      @PathVariable(APP_NAME_PARAM) String appName,
    ...
    checkAndCreateGroup(
        appSecret.getGroup(), createGroup, user); // Check and create group if not exists.

    CreateSecretRequestV2 secretRequestV2 =
        CreateSecretRequestV2.fromParts(
            uniqSecretName,
            secret.getContent(),
    ...
    auditLog.log(
        new Event(
            SECRET_CREATE,
            user.getUsername(),
            appSecret.getGroupName(),
            appSecret.getSecretName()));
  ...
  public SecretDetailResponseV2 getSecret(
      @PathVariable(APP_NAME_PARAM) String appName,
      @PathVariable(APP_SECRET_PARAM) String secretName,
      @CurrentUser OneOpsUser user)
      throws IOException {
   ...
   * @throws IOException Throws if the request could not be executed due to cancellation, a
   *     connectivity problem or timeout.
   * @throws KeywhizException Throws if the secret not exists or is not part of given application
   *     group.
   */
  ...
  public List<SecretDetailResponseV2> getSecretVersions(
    ...
    checkSecretInGroup(appSecret);
    List<SecretDetailResponseV2> secrets =
        kwClient.getSecretVersions(uniqSecretName, 0, Integer.MAX_VALUE);
    return secrets.stream().map(secretService::normalize).collect(Collectors.toList());
  }
   ...
   * @throws IOException Throws if the request could not be executed due to cancellation, a
   *     connectivity problem or timeout.
   * @throws KeywhizException Throws if the secret not exists or not part of given application
   *     group.
   */
  ...
  /**
   * Retrieve contents for a set of secret series. This is a POST request because it updates the
  ...
  private void purgeSecret(AppSecret appSecret, @CurrentUser OneOpsUser user) throws IOException {
    ...
    kwClient.deleteSecret(uniqSecretName);
    auditLog.log(
        new Event(
            SECRET_DELETE,
            user.getUsername(),
  ...
  private void checkClientInGroup(String clientName, AppGroup appGroup) throws IOException {
    ...
    boolean clientExists =
        clients.stream().anyMatch(client -> client.name().equalsIgnoreCase(clientName));
    if (!clientExists) {
  ...
  private void checkSecretInGroup(AppSecret appSecret) throws IOException {
    ...
    if (!groupExists) {
      ...
      throw new KeywhizException(
          NOT_FOUND.value(), "Secret " + appSecret.getSecretName() + " not found.");
    }
  }
   ...
   */
  private void checkAndCreateGroup(AppGroup appGroup, boolean createGroup, OneOpsUser user)
      throws IOException {
    ...
    try {
      ...
      if (createGroup) {
        ...
        throw new KeywhizException(
            NOT_FOUND.value(), format("Application group, %s not exists.", group), ex);
      }
    }
  }
}
